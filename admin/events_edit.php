<?php require_once("../included.php");
	...
	$index = $_REQUEST['id'];
	$query = mysqli_query($conn, "SELECT * FROM `activities` WHERE `id`='$index' LIMIT 1");
	$results = mysqli_fetch_array($query);
	  if($_REQUEST['title']){
			...
			mysqli_query($conn, "UPDATE `activities` SET `name`='$title', `date`='$date',  `desc`='$description', `limit`='$limit', `time`='$time', `location`='$location'  WHERE `id`='$index' ") or DIE(mysqli_error($conn));
			...
			<div id="content">
				...
				<div id="mainContent">
					...
					<p>You can use these html tags while writing the entry.</p>					
						...
						<?php 
							$string = "events_edit.php?id=$index";
						?>
					<form id="submitEvents" method="post" action="<?php echo $string; ?>">
						<label for="title" class="left">Title</label><br />
						<input type="text" id="title"  name="title" value="<?php echo $results['name']; ?>"/>						
						<label for="date" class="left">Date (MM/DD/YYYY)</label><br />
						<input type="hidden" id="unixtime" name="unixtime" value="<?php echo $results['date']; ?>" />
						<script>
							$(document).ready(function() {
								var value = $("#unixtime").val();
								var dateStamp = new Date(value*1000);
								var dateString = moment(dateStamp).calendar();
								$("#date").val(dateString);
							});
						</script>
						<input type="text" id="date"  name="date" />						
						<label for="limit" class="left">Event Limit</label>
						<input type="text" id="limit" name="limit"  value="<?php echo $results['limit']; ?>"/>
						<label for="time" class="left">Event Time</label>
						<input type="text" id="time" name="time" value="<?php echo $results['time']; ?> "/>
						<label for="location" class="left">Event Location</label> <br />
						<input type="text" id="location" name="location" value="<?php echo $results['location']; ?>" />
						<label for="eventDesc" class="left">Description</label> <br />
						<textarea id="eventDesc" name="eventDesc" ><?php echo htmlspecialchars($results['desc']); ?></textarea>
						<div class="row">
